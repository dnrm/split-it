...
DO $$ 
BEGIN
    -- Drop ticket-related policies first (they reference functions and other tables)
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Group members can view claims from their group tickets' AND tablename = 'ticket_item_claims') THEN
        DROP POLICY IF EXISTS "Group members can view claims from their group tickets" ON public.ticket_item_claims;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Group members can create claims' AND tablename = 'ticket_item_claims') THEN
        DROP POLICY IF EXISTS "Group members can create claims" ON public.ticket_item_claims;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update their own claims' AND tablename = 'ticket_item_claims') THEN
        DROP POLICY IF EXISTS "Users can update their own claims" ON public.ticket_item_claims;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete their own claims' AND tablename = 'ticket_item_claims') THEN
        DROP POLICY IF EXISTS "Users can delete their own claims" ON public.ticket_item_claims;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Group members can view items from their group tickets' AND tablename = 'ticket_items') THEN
        DROP POLICY IF EXISTS "Group members can view items from their group tickets" ON public.ticket_items;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Group members can create ticket items' AND tablename = 'ticket_items') THEN
        DROP POLICY IF EXISTS "Group members can create ticket items" ON public.ticket_items;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Group members can update ticket items' AND tablename = 'ticket_items') THEN
        DROP POLICY IF EXISTS "Group members can update ticket items" ON public.ticket_items;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Group members can delete ticket items' AND tablename = 'ticket_items') THEN
        DROP POLICY IF EXISTS "Group members can delete ticket items" ON public.ticket_items;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Group members can view tickets from their groups' AND tablename = 'shared_tickets') THEN
        DROP POLICY IF EXISTS "Group members can view tickets from their groups" ON public.shared_tickets;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Group members can create tickets' AND tablename = 'shared_tickets') THEN
        DROP POLICY IF EXISTS "Group members can create tickets" ON public.shared_tickets;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Ticket uploaders can update their tickets' AND tablename = 'shared_tickets') THEN
        DROP POLICY IF EXISTS "Ticket uploaders can update their tickets" ON public.shared_tickets;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Ticket uploaders can delete their tickets' AND tablename = 'shared_tickets') THEN
        DROP POLICY IF EXISTS "Ticket uploaders can delete their tickets" ON public.shared_tickets;
    END IF;
...
-- Drop triggers first (before dropping tables they depend on)
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP TRIGGER IF EXISTS on_group_created ON public.groups;

-- Drop tables (in reverse dependency order) - only if they exist
DROP TABLE IF EXISTS public.ticket_item_claims CASCADE;
DROP TABLE IF EXISTS public.ticket_items CASCADE;
DROP TABLE IF EXISTS public.shared_tickets CASCADE;
...
-- Drop functions - now safe to drop because all dependent tables and policies are gone
DROP FUNCTION IF EXISTS public.handle_new_user();
DROP FUNCTION IF EXISTS public.handle_new_group();
DROP FUNCTION IF EXISTS public.is_group_member(UUID, UUID) CASCADE;
DROP FUNCTION IF EXISTS public.get_ticket_claim_summary(UUID) CASCADE;
DROP FUNCTION IF EXISTS public.get_user_ticket_total(UUID, UUID) CASCADE;
